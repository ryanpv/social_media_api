...
def test_create_access_token():  # paramterize???
    token = security.create_access_token("123")
    assert {"sub": "123", "type": "access"}.items() <= jwt.decode(
        token, key=config.PWD_SECRET_KEY, algorithms=[security.ALGORITHM]
    ).items()
...
def test_get_subject_for_token_type_missing_sub():
    ...
    del payload["sub"]
    token = jwt.encode(payload, key=config.PWD_SECRET_KEY, algorithm=security.ALGORITHM)
...
async def test_authenticate_user_not_found():
    with pytest.raises(security.HTTPException):
        await security.authenticate_user("test@example.com", "12345678")
